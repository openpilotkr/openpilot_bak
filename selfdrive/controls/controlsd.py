...
class Controls:
  ...
  def publish_logs(self, CS, start_time, actuators, lac_log):
    ...
    if len(speeds) > 1:
      ...
      v_future = 100.0
    CC.hudControl.vFuture = float(v_future * CV.MS_TO_KPH)
    ...
    if self.enabled and CS.cruiseState.available:
      self.hkg_stock_lkas = False
      self.hkg_stock_lkas_timer = 0
    elif not self.enabled and not CS.cruiseState.available and not self.hkg_stock_lkas:
