...
class LongControl():
  ...
  def update(self, active, CS, CP, long_plan, accel_limits, radarState):
    ...
    if CP.sccBus != 0 and self.long_log:
      str_log3 = 'LS={:s}  LP={:s}  FACC={:01.2f}  GS={}  ED/RD={:04.1f}/{:04.1f}  TG={:04.2f}/{:04.2f}/{:+04.2f}'.format(self.long_stat, self.long_plan_source, final_accel, int(CS.gasPressed), dRel, CS.radarDistance, v_target, v_target_future, a_target)
