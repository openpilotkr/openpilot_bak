  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, visual_alert,
    ...
    if CS.out.cruiseState.accActive:
      str_log2 = 'AQ={:+04.2f}  VF={:03.0f}  TS={:03.0f}  SS={:03.0f}  CS={:03.0f}  RD/RV={:04.1f}/{:03.1f}  CG={:1.0f}  FR={:03.0f}'.format(
       self.aq_value if self.longcontrol else CS.scc12["aReqValue"], v_future, self.NC.ctrl_speed , setSpeed, round(CS.out.cruiseState.speed), CS.lead_distance, CS.lead_objspd, CS.cruiseGapSet, self.timer1.sampleTime())
    else:
